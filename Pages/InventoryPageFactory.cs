...
namespace Managing
{
    ...
    public class InventoryPageFactory : ContentPage
    {
        ...
        private async void OnPageAppearing(object sender, EventArgs e)
        {
            try
            {
                ...
                _stockItemsView.ItemTemplate = new DataTemplate(() =>
                {
                    if (DeviceInfo.Platform == DevicePlatform.Android || DeviceInfo.Platform == DevicePlatform.iOS)
                    {
                        // Flat rectangle card, no border radius, no shadow
                        var card = new Frame
                        {
                            CornerRadius = 0,
                        ...
                        var grid = new Grid
                        {
                            ColumnDefinitions =
                            {
                                new ColumnDefinition { Width = new GridLength(1, GridUnitType.Star) }, // Product info
                                new ColumnDefinition { Width = GridLength.Auto }, // Quantity
                        ...
                        // Quantity value (less vertical space)
                        var qtyValueLabel = new Label
                        {
                            FontSize = 20,
                            TextColor = Colors.Black,
                            HorizontalOptions = LayoutOptions.End,
                        ...
                        qtyValueLabel.SetBinding(Label.TextProperty, "Quantity");
                        grid.Add(qtyValueLabel, 1, 1);

                        // Edit/Delete buttons, vertically stacked, smaller
        ...
        private async void OnEditItemClicked(object sender, EventArgs e)
        {
            try
            {
                // Check for both Button and ImageButton types
                StockItemViewModel item = null;
                if (sender is Button btn && btn.CommandParameter is StockItemViewModel btnItem)
                {
                    item = btnItem;
                }
                else if (sender is ImageButton imgBtn && imgBtn.CommandParameter is StockItemViewModel imgBtnItem)
                {
                    item = imgBtnItem;
                }
        ...
        private async void OnDeleteItemClicked(object sender, EventArgs e)
        {
            try
            {
                ...
                if (item != null)
                {
                    ...
                    // Delete the entire item instead of just consuming its quantity
                    await stockService.DeleteStockAsync(item.SKU, $"Item removido pelo utilizador: {item.ProductName}");
